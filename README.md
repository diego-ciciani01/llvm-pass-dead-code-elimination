...
A dead code elimination pass for llvm, implemented as a loadable plugin for the Pass Menager. The pass implement three families of optimization, that reinformce them self through a fixedpoint driver:
...
- **Remove unrechable blocks** Reachability of blocks is tested with depth-firs-search. Unreachable block may still be referanced, so before erasing them:
...
- **Phase 2** (tests/fold/): same - target branch fold, a fold that bails out on the successor PHI, an unrechable-block test with cross referances with two dead blocks
and an empty-block bypass, this two below (tests/).
